dynare_location='C:\dynare\4.6.4\matlab';
...
eig_seperation_exact=min(min(chordal_dist));
...
D_d=subs(D,{h beta delta alpha sigma rho omega},{Y(1) Y(2) Y(3) Y(4) Y(5) Y(6) Y(7)});
inputs.A=double(A_d);
inputs.B=double(B_d);
inputs.C=double(C_d);
...
D_d=subs(D,{h beta delta alpha sigma rho omega},{Y(1) Y(2) Y(3) Y(4) Y(5) Y(6) Y(7)});
inputs.A=double([A_d zeros(2,1);0 0 0]);
inputs.B=double([B_d D_d;0 0 -1]);
inputs.C=double([C_d zeros(2,1);0 0 Y(6)]);
inputs.X=double([X_stable double(Q_exact)*Y(6); 0 0 Y(6)]);
D_d=subs(D,{h beta delta alpha sigma rho omega},{Y(1) Y(2) Y(3) Y(4) Y(5) Y(6) Y(7)});
inputs.D=[0;0;1];
inputs.W=double([Q_exact; 1]);
[impact_errors_exact] = matrix_impact_backward_errors_vector(inputs)
 save('exact_results','rp_exact','X_exact','Q_exact','log_c_std_exact','eig_seperation_exact','eigval_exact','errors_exact','impact_errors_exact')
...
l_2=eigval(abs(eigval)<1);
chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
eig_seperation_uhlig=min(min(chordal_dist));
load('Y')
 Y(1)=Y_1(1);Y(2)=0.99;Y(3)=0.025;Y(4)=0.36;Y(5)=Y_1(2);Y(6)=0.95;Y(7)=Y_1(3);
 %Y=Y_1;
 h=Y(1);
...
inputs.A=[A zeros(2,1);0 0 0];
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_uhlig BETA_ZS_uhlig*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_uhlig; 1];
[impact_errors_uhlig] = matrix_impact_backward_errors_vector(inputs)
save('uhlig_results','rp_uhlig','ALPHA_ZS_uhlig','BETA_ZS_uhlig','log_c_std_uhlig','eig_seperation_uhlig','eigval_uhlig','errors_uhlig','impact_errors_uhlig')
rmpath('uhlig')
...
l_2=eigval(abs(eigval)<1);
chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
eig_seperation_aim=min(min(chordal_dist));
inputs.A=A;
inputs.B=B;
inputs.C=C;
...
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_aim BETA_ZS_aim*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_aim; 1];
[impact_errors_aim] = matrix_impact_backward_errors_vector(inputs)
save('aim_results','rp_aim','ALPHA_ZS_aim','BETA_ZS_aim','log_c_std_aim','eig_seperation_aim','eigval_aim','errors_aim','impact_errors_aim')
...
l_2=eigval(abs(eigval)<1);
chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
eig_seperation_solab=min(min(chordal_dist));
[sortabs,sortindex] = sort(abs(eigval));
...
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_solab BETA_ZS_solab*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_solab; 1];
[impact_errors_solab] = matrix_impact_backward_errors_vector(inputs)
 save('solab_results','rp_solab','ALPHA_ZS_solab','BETA_ZS_solab','log_c_std_solab','eig_seperation_solab','eigval_solab','errors_solab','impact_errors_solab')
...
l_2=eigval(abs(eigval)<1);
chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
eig_seperation_gensys=min(min(chordal_dist));
[sortabs,sortindex] = sort(abs(eigval));
...
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_gensys BETA_ZS_gensys*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_gensys; 1];
[impact_errors_gensys] = matrix_impact_backward_errors_vector(inputs)
 save('gensys_results','rp_gensys','ALPHA_ZS_gensys','BETA_ZS_gensys','log_c_std_gensys','eig_seperation_gensys','eigval_gensys','errors_gensys','impact_errors_gensys')
rmpath('gensys') 
...
l_2=eigval(abs(eigval)<1);
chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
eig_seperation_bp=min(min(chordal_dist));
[sortabs,sortindex] = sort(abs(eigval));
...
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_bp BETA_ZS_bp*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_bp; 1];
[impact_errors_bp] = matrix_impact_backward_errors_vector(inputs)
 save('bp_results','rp_bp','ALPHA_ZS_bp','BETA_ZS_bp','log_c_std_bp','eig_seperation_bp','eigval_bp', 'errors_bp','impact_errors_bp')
...
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_dynare BETA_ZS_dynare*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_dynare; 1];
[impact_errors_dynare] = matrix_impact_backward_errors_vector(inputs)
 save('dynare_results','rp_dynare','ALPHA_ZS_dynare','BETA_ZS_dynare','log_c_std_dynare','eig_seperation_dynare','eigval_dynare','errors_dynare','impact_errors_dynare')
     path(p); 
 clear all
 p = path; 
 load('dynare_location')
...
C=[-sigma*h/(1-h) 0;0 (1-delta+alpha*K_aminus1)];
D=[beta*alpha*K_aminus1*rho; K_aminus1];
eig_s=eig(ALPHA_ZS_dynare_cr)
X_unstable=-(B+A*ALPHA_ZS_dynare_cr)\A;
eig_u=eig(X_unstable);
eig_u=1./eig_u
%eig_seperation=eig_u-eig_s';[a b]=min(abs(eig_seperation(:)));eig_seperation_dynare_cr=eig_seperation(b)
eigval=[eig_s;eig_u];
l_1=eigval(abs(eigval)>1);
l_2=eigval(abs(eigval)<1);
chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
...
[sortabs,sortindex] = sort(abs(eigval));
eigval_dynare_cr=eigval(sortindex);
inputs.A=A;
inputs.B=B;
inputs.C=C;
...
inputs.B=[B D;0 0 -1];
inputs.C=[C zeros(2,1);0 0 rho];
inputs.X=[ALPHA_ZS_dynare_cr BETA_ZS_dynare_cr*rho; 0 0 rho];
inputs.D=[0;0;1];
inputs.W=[BETA_ZS_dynare_cr; 1];
[impact_errors_dynare_cr] = matrix_impact_backward_errors_vector(inputs)
 save('dynare_cr_results','rp_dynare_cr','ALPHA_ZS_dynare_cr','BETA_ZS_dynare_cr','log_c_std_dynare_cr','eig_seperation_dynare_cr','eigval_dynare_cr','errors_dynare_cr','impact_errors_dynare_cr')
...
results=nan(5,9);
 ...
 impact_results(:,1)=[impact_errors_exact(3,:)'; impact_errors_exact(1,:)'; impact_errors_exact(2,:)'];
 load('solab_results')
 ...
 num_results(:,2)=[eig_seperation_solab; errors_solab.conditioning; errors_solab.lower_backward_error; errors_solab.upper_backward_error];
  impact_results(:,2)=[impact_errors_solab(3,:)'; impact_errors_solab(1,:)'; impact_errors_solab(2,:)'];
load('gensys_results')
 ...
 impact_results(:,3)=[impact_errors_gensys(3,:)'; impact_errors_gensys(1,:)'; impact_errors_gensys(2,:)'];
load('uhlig_results')
...
eig_results(:,4)=[eig_seperation_uhlig; eigval_exact-eigval_uhlig];
 ...
 impact_results(:,4)=[impact_errors_uhlig(3,:)'; impact_errors_uhlig(1,:)'; impact_errors_uhlig(2,:)'];
load('dynare_results')
...
eig_results(:,5)=[eig_seperation_dynare; eigval_exact-eigval_dynare];
 ...
 impact_results(:,5)=[impact_errors_dynare(3,:)'; impact_errors_dynare(1,:)'; impact_errors_dynare(2,:)'];
load('aim_results')
...
eig_results(:,6)=[eig_seperation_aim; eigval_exact-eigval_aim];
 ...
 impact_results(:,6)=[impact_errors_aim(3,:)'; impact_errors_aim(1,:)'; impact_errors_aim(2,:)'];
load('bp_results') 
...
impact_results(:,7)=[impact_errors_bp(3,:)'; impact_errors_bp(1,:)'; impact_errors_bp(2,:)'];
...
eig_results(:,8)=[eig_seperation_dynare_cr; eigval_exact-eigval_dynare_cr];
 ...
 impact_results(:,8)=[impact_errors_dynare_cr(3,:)'; impact_errors_dynare_cr(1,:)'; impact_errors_dynare_cr(2,:)'];
