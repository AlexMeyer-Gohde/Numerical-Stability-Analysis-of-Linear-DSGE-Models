...
load('exact_results'); cs=1; errors_w=errors_exact;rp_w=rp_exact;log_c_sw=log_c_std_exact;
results_P([1 3],cs)=[rp_w; 100*log_c_sw]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ([1 3],cs)=[rp_w; 100*log_c_sw];
results_PQ(6:end,cs)=errors_w(P_select,5);
results_PQ(9,cs)=results_P(9,cs);
results_Q(3:end,cs)=errors_w(Q_select);
eig_sel=~isinf(eigval_exact);
eigval_exact_cp=double(eigval_exact(eig_sel));

load('solab_results');cs=2; errors_w=errors_solab;rp_w=rp_solab;log_c_sw=log_c_std_solab;P_w=ALPHA_ZS_solab;Q_w=BETA_ZS_solab;eigval_w=eigval_solab(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
results_PQ(9,cs)=results_P(9,cs);
results_Q(1:2,cs)=[rp_exact-rp_w;  double(max(max(abs(Q_exact-Q_w))))]; 
results_Q(3:end,cs)=errors_w(Q_select);

load('gensys_results');cs=3; errors_w=errors_gensys;rp_w=rp_gensys;log_c_sw=log_c_std_gensys;P_w=ALPHA_ZS_gensys;Q_w=BETA_ZS_gensys;eigval_w=eigval_gensys(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
...
load('uhlig_results');cs=4; errors_w=errors_uhlig;rp_w=rp_uhlig;log_c_sw=log_c_std_uhlig;P_w=ALPHA_ZS_uhlig;Q_w=BETA_ZS_uhlig;eigval_w=eigval_uhlig(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
...
load('dynare_results');cs=5; errors_w=errors_dynare;rp_w=rp_dynare;log_c_sw=log_c_std_dynare;P_w=ALPHA_ZS_dynare;Q_w=BETA_ZS_dynare;eigval_w=eigval_dynare(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
...
load('aim_results');cs=6; errors_w=errors_aim;rp_w=rp_aim;log_c_sw=log_c_std_aim;P_w=ALPHA_ZS_aim;Q_w=BETA_ZS_aim;eigval_w=eigval_aim(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
...
load('bp_results');cs=7; errors_w=errors_bp;rp_w=rp_bp;log_c_sw=log_c_std_bp;P_w=ALPHA_ZS_bp;Q_w=BETA_ZS_bp;eigval_w=eigval_bp(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
...
load('dynare_cr_results');cs=8; errors_w=errors_dynare_cr;rp_w=rp_dynare_cr;log_c_sw=log_c_std_dynare_cr;P_w=ALPHA_ZS_dynare_cr;Q_w=BETA_ZS_dynare_cr;eigval_w=eigval_dynare_cr(eig_sel);
results_P(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs(X_exact-P_w)))); max(abs(eigval_exact_cp-eigval_w))]; 
results_P(6:end,cs)=errors_w(P_select,1);
results_PQ(1:5,cs)=[rp_w; rp_exact-rp_w; 100*log_c_sw; double(max(max(abs([X_exact Q_exact]-[P_w Q_w])))); max(abs(eigval_exact_cp-eigval_w))];
results_PQ(6:end,cs)=errors_w(P_select,5);
...
results_short=[results_PQ(10,1) results_PQ(2,2:end); results_PQ(11,1)  results_PQ(13,2:end)];
