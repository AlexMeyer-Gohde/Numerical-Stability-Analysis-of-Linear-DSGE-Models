...
results(3,1)=1/svds(H_P,1,'smallest');

%Q_1 H+2
H_Q1=[phi*kron(Q',speye(ny)) delta*speye(ny*ne)];
results(3,2)=1/svds(H_Q1,1,'smallest');

%Q_2 H+2
H_Q2=[alpha*kron(P_Q',speye(ny)) beta*kron(Q',speye(ny)) delta*speye(ny*ne)];
results(3,3)=1/svds(H_Q2,1,'smallest');

%Q_3 H+2
results(3,4)=results(3,3);
...
results(3,5)=1/svds(H_PQ,1,'smallest');

%H+2 est
%P H+2
results(4,1)=normest(pinv(H_P));

%Q_1 H+2
results(4,2)=normest(pinv(H_Q1));

%Q_2 H+2
results(4,3)=normest(pinv(H_Q2));

%Q_3 H+2
results(4,4)=results(4,3);

%PQ H+2
results(4,5)=normest(pinv(H_PQ));
...
%PQ diff-1 est
[A1,D1,Pz,Qz] = qz(full(F),full(A));
[U,B1] = schur(full(-[[P';Q'],sparse(ny+ne,ne)]),'complex');
...
%psi
%P psi
%temp_P=[alpha*kron(P_2',eye(ny)) beta*kron(P',speye(ny)) gamma*speye(ny^2) sparse(ny*ne,ny*ne)];
temp_P=V\H_P;
results(10,1)=norm(temp_P)/P_F;
results(11,1)=normest(temp_P)/P_F;
%results(4,1)=normest(H_P\[alpha*kron(P_2',speye(ny)) beta*kron(P',speye(ny)) gamma*speye(ny^2)])/P_F;
...
%phi
%P phi
%results(5,1)=(1/(min_svd_F+min_svd_P*min_svd_A)^(1/2))*normest([alpha*kron(P_2',speye(ny)) beta*kron(P',speye(ny)) gamma*speye(ny^2)])/P_F;
...
results(13,5)=results(7,1)*(alpha*(P_2_F+P_Q_F)+beta*(P_F+Q_F)+(gamma+delta))/m_P_Q_F;%CHECK;%results(7,5)*(alpha*(P_2_F+P_Q_F)+beta*(P_F+Q_F)+(gamma+delta))/m_P_Q_F;%CHECK


%FE
%P FE
temp_P=V\R_P(:);
results(14,1)=norm(temp_P)/P_F;
results(15,1)=results(7,1)*(R_P_F/P_F); %CHECK

%Q_1 FE
temp_Q=I_F\R_Q(:);
results(14,2)=norm(temp_Q)/Q_F;
results(15,2)=results(7,2)*(R_Q_F/Q_F); %CHECK
...
results(14,4)=norm(temp,1)/Q_F;
%results(17,4)=results(17,2)+results(7,2)*results(17,1)*svds(V,1,'largest'); %CHECK
results(15,4)=results(7,2)*((R_Q_F/Q_F)+results(7,1)*A_2*R_P_F);

%PQ FE
temp=[kron([speye(ny);sparse(ne,ny)],speye(ny)) kron([sparse(ny,ne);speye(ne)],speye(ny))]*[R_P(:);R_Q(:)];
results(14,5)=norm(W\temp)/m_P_Q_F;
results(15,5)=results(7,1)*R_P_Q_F/m_P_Q_F;%results(7,5)*R_P_Q_F/m_P_Q_F;
