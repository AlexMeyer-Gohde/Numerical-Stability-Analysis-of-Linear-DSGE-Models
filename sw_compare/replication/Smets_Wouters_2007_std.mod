...
load('sw_parameterization');
        for i = 1:length(total_x)
            parameter_index=x_location(i,1);
            paramname = char(deblank(x_names(parameter_index,:)));
            eval(['set_param_value(paramname,total_x(' int2str(i) '));']);
        end
...
targets=[2.899 0.867 0.673 1.014 0.841 2.614 0.622];
