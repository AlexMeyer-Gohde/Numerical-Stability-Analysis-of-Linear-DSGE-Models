...
loop_n=size(mmb_vec,1);
 ...
 %num_results=nan(4,6,loop_n);
 %impact_results=nan(9,7,loop_n);
...
% [errors]=matrix_quadratic_backward_errors_vector(matrix_quadratic)
% [impact_errors] = matrix_impact_backward_errors_vector(matrix_quadratic)
...
% num_results(:,4,loop_k)=[eig_seperation_dynare;errors];
% impact_results(:,4,loop_k)=[impact_errors(3,:)'; impact_errors(1,:)'; impact_errors(2,:)'];
% 
...
% matrix_quadratic.W=BETA_ZS_aim;
% [errors]=matrix_quadratic_backward_errors_vector(matrix_quadratic)
% [impact_errors] = matrix_impact_backward_errors_vector(matrix_quadratic)
% eigval=oo_.dr.eigval;
% l_1=eigval(abs(eigval)>1);
...
% chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
% eig_seperation_dynare=min(min(chordal_dist));
% num_results(:,5,loop_k)=[eig_seperation_dynare;errors];
% impact_results(:,5,loop_k)=[impact_errors(3,:)'; impact_errors(1,:)'; impact_errors(2,:)'];
% 
% try
...
% matrix_quadratic.W=BETA_ZS_solab;
% [errors]=matrix_quadratic_backward_errors_vector(matrix_quadratic)
% [impact_errors] = matrix_impact_backward_errors_vector(matrix_quadratic)
% eigval=eigval_solab;
% l_1=eigval(abs(eigval)>1);
...
% num_results(:,1,loop_k)=[eig_seperation;errors];
% impact_results(:,1,loop_k)=[impact_errors(3,:)'; impact_errors(1,:)'; impact_errors(2,:)'];
% end
% 
% try
% addpath([YourPath '\gensys'])
...
% matrix_quadratic.W=BETA_ZS_gensys;
% [errors]=matrix_quadratic_backward_errors_vector(matrix_quadratic)
% [impact_errors] = matrix_impact_backward_errors_vector(matrix_quadratic)
% eigval=gev(:,2)./gev(:,1);
% l_1=eigval(abs(eigval)>1);
...
% chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
% eig_seperation=min(min(chordal_dist));
% num_results(:,2,loop_k)=[eig_seperation;errors];
% impact_results(:,2,loop_k)=[impact_errors(3,:)'; impact_errors(1,:)'; impact_errors(2,:)'];
% rmpath([YourPath '\gensys'])
% end
% 
% try
% addpath([YourPath '\uhlig'])
% 
% AA=[]; BB=[]; CC=[]; DD=[];
...
% matrix_quadratic.W=BETA_ZS_uhlig;
% [errors]=matrix_quadratic_backward_errors_vector(matrix_quadratic)
% [impact_errors] = matrix_impact_backward_errors_vector(matrix_quadratic)
% eigval=Xi_sortval;
% l_1=eigval(abs(eigval)>1);
...
% chordal_dist=abs(l_1-l_2')./((abs(l_1).^2+1).^(1/2).*(abs(l_2').^2+1).^(1/2));
% eig_seperation=min(min(chordal_dist));
% num_results(:,3,loop_k)=[eig_seperation;errors];
% impact_results(:,3,loop_k)=[impact_errors(3,:)'; impact_errors(1,:)'; impact_errors(2,:)'];
% rmpath([YourPath '\uhlig'])
% end
 try
...
save Fourth_Run_AMG_JS_only_cr_add
end
